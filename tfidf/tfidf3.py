...
def make_set(ld):
	corpus = []
	#print_err("set: %s"%(", ".join(ld)))
	for filename in ld:
		doc = read_docs(filename, 2)
...
parser.add_argument('-1', '--group1', help='first list of docs', nargs='+')
parser.add_argument('-2', '--group2', help='second list of docs', nargs='+')
...
doc1 = make_set(args.group1)
doc2 = make_set(args.group2)
